...
class QLearningParameters:
    ...
    temperature: float = 5.0         # Increased initial temperature to encourage exploration
    temperature_decay: float = 0.999  # Decay per episode
    min_temperature: float = 1.0      # Minimum allowed temperature
...
def train_agents():
    ...
    for episode in range(1, simulation_params.episodes + 1):
        ...
        for step in range(simulation_params.max_steps_per_episode):
            ...
            if not agent1_reached_goal:
                ...
                if (new_agent1_pos != agent1_pos and new_agent1_pos != last_state_agent1 and
                    ...
                    reward_agent1 = calculate_reward(
                        ...
                        other_agent_pos=agent2_pos,  # For simplicity, compare with Agent 2
            ...
            if (manhattan_distance(agent1_pos, agent2_pos) < 2 or
                manhattan_distance(agent1_pos, agent3_pos) < 2 or
                manhattan_distance(agent2_pos, agent3_pos) < 2):
                collision_count += 1
            ...
            if (total_reward_agent1 < penalties.min_cumulative or
                ...
                total_reward_agent3 < penalties.min_cumulative):
...
def animate_simulation(paths, title):
    ...
    gif_filename = "Three_Agents_Plots_Boltz/" + title.replace(" ", "_") + ".gif"
...
def plot_heatmap(
    ...
    fig.tight_layout()
    fn = "Three_Agents_Plots_Boltz/" + title.replace(" ", "_") + ".png"
...
def enhanced_risk_analysis(burn_in: int = 0):
    ...
    max_q_agents = [
        ...
        np.flip(np.max(Q_table_agent3, axis=2))
    ]
...
def plot_learning_curves():
    ...
    for agent in agents:
        ...
        if rewards_array.size > 0:
            window = simulation_params.max_steps_per_episode
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Three_Agents_Plots_Boltz/Learning_Curves.pdf", format="pdf")
    plt.show()
