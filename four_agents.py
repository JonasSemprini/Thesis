...
class GridSettings:
    ...
    agent4_start: Tuple[int, int] = (9, 9)         # Bottom-right corner (row 9, col 9)
    
    agent1_victory_zone: Tuple[int, int] = (9, 0)   # Agent 1 aims for bottom-left
    agent2_victory_zone: Tuple[int, int] = (0, 9)   # Agent 2 aims for top-right
    agent3_victory_zone: Tuple[int, int] = (9, 9)   # Agent 3 aims for bottom-right
    agent4_victory_zone: Tuple[int, int] = (0, 0)   # Agent 4 aims for top-left
...
class QLearningParameters:
    ...
    exploration_rate: float = 1.0  # (Not used with Boltzmann but kept for reference.)
...
Q_table_agent4 = np.zeros((grid_settings.size, grid_settings.size, ql_params.num_actions))
...
total_visits_agent4 = np.zeros((grid_settings.size, grid_settings.size), dtype=int)
...
def calculate_reward(agent_pos: Tuple[int, int],
    ...
    # Add repulsion: if any other agent is too close, subtract a penalty.
    safe_distance = 6  # agents should keep at least this Manhattan distance
    repulsion_weight = 50  
    for pos in other_agents:
        d = manhattan_distance(agent_pos, pos)
        if d < safe_distance and d > 0:
            reward -= repulsion_weight / d
    return reward


def boltzmann_action_selection(state: Tuple[int, int],
                               Q_table: np.ndarray,
                               temperature: float,
                               last_state: Tuple[int, int],
    ...
    if not valid_actions:
        return -1
    #With a small probability (e.g., 1%), choose a random action to boost exploration.
    if np.random.rand() < 0.01:
        return np.random.choice(valid_actions)

    q_values = Q_table[state[0], state[1], valid_actions]
...
def train_agents():
    ...
    global total_visits_agent1, total_visits_agent2, total_visits_agent3, total_visits_agent4
    ...
    for episode in range(1, simulation_params.episodes+1):
        ...
        for step in range(simulation_params.max_steps_per_episode):
            ...
            visited_states_agent4[agent4_pos] += 1

           
            if not agent1_reached_goal:
                current_state_agent1 = agent1_pos
                action_agent1 = boltzmann_action_selection(
                    state=agent1_pos,
                    Q_table=Q_table_agent1,
                    temperature=ql_params.temperature,
                ...
                if (new_agent1_pos != agent1_pos and new_agent1_pos != last_state_agent1 and
                    ...
                    reward_agent1 = calculate_reward(
                        ...
                        other_agents=[agent2_pos, agent3_pos, agent4_pos],
            ...
            if not agent2_reached_goal:
                ...
                action_agent2 = boltzmann_action_selection(
                    state=agent2_pos,
                ...
                if (new_agent2_pos != agent2_pos and new_agent2_pos != last_state_agent2 and
                    ...
                    reward_agent2 = calculate_reward(
                        ...
                        other_agents=[agent1_pos, agent3_pos, agent4_pos],
            ...
            if not agent3_reached_goal:
                ...
                action_agent3 = boltzmann_action_selection(
                    state=agent3_pos,
                ...
                if (new_agent3_pos != agent3_pos and new_agent3_pos != last_state_agent3 and
                    ...
                    reward_agent3 = calculate_reward(
                        ...
                        other_agents=[agent1_pos, agent2_pos, agent4_pos],
            ...
            if not agent4_reached_goal:
                current_state_agent4 = agent4_pos
                action_agent4 = boltzmann_action_selection(
                    state=agent4_pos,
                    Q_table=Q_table_agent4,
                    temperature=ql_params.temperature,
                    last_state=last_state_agent4,
                    actions=ql_params.actions,
                    grid_size=grid_settings.size
                )
                if action_agent4 == -1:
                    logging.debug(f"Episode {episode}: Agent 4 has no valid actions.")
                    break
                dx4, dy4 = ql_params.actions[action_agent4]
                new_agent4_pos = (agent4_pos[0] + dx4, agent4_pos[1] + dy4)
                if (new_agent4_pos != agent4_pos and new_agent4_pos != last_state_agent4 and
                    0 <= new_agent4_pos[0] < grid_settings.size and 0 <= new_agent4_pos[1] < grid_settings.size):
                    last_state_agent4 = agent4_pos
                    agent4_pos = new_agent4_pos
                    reward_agent4 = calculate_reward(
                        agent_pos=agent4_pos,
                        other_agents=[agent1_pos, agent2_pos, agent3_pos],
                        victory_zone=grid_settings.agent4_victory_zone,
                        critical_zone=critical_zone,
                        last_state=last_state_agent4,
                        visited_states=visited_states_agent4,
                        has_reached_goal=agent4_reached_goal,
                        rewards=rewards,
                        penalties=penalties)
                    total_reward_agent4 += reward_agent4
                    episode_path_agent4.append(agent4_pos)
                    cumulative_rewards_agent4.append(total_reward_agent4)
                else:
                    cumulative_rewards_agent4.append(total_reward_agent4)
                if agent4_pos == grid_settings.agent4_victory_zone:
                    agent4_reached_goal = True
                    logging.debug(f"Episode {episode}: Agent 4 reached its victory zone.")

            
            cs = grid_settings.collision_threshold
            if (manhattan_distance(agent1_pos, agent2_pos) < cs or
                manhattan_distance(agent1_pos, agent3_pos) < cs or
                manhattan_distance(agent1_pos, agent4_pos) < cs or
                manhattan_distance(agent2_pos, agent3_pos) < cs or
                manhattan_distance(agent2_pos, agent4_pos) < cs or
                manhattan_distance(agent3_pos, agent4_pos) < cs):
                collision_count += 1
                ...
                total_reward_agent4 += collision_penalty
                ...
                cumulative_rewards_agent4[-1] += collision_penalty
                collision_paths.append(CollisionPath(
                    ...
                    agent4_run=(episode_path_agent4, cumulative_rewards_agent4.copy())
                ))
                logging.info(f"Episode {episode}: Collision occurred among agents at positions {agent1_pos}, {agent2_pos}, {agent3_pos}, {agent4_pos}.")
                break
            else:
                ...
                if not agent3_reached_goal and action_agent3 != -1:
                    update_q_table(Q_table=Q_table_agent3, state=current_state_agent3, action=action_agent3,
                                   ...
                                   learning_rate=ql_params.learning_rate, discount_factor=ql_params.discount_factor)
                if not agent4_reached_goal and action_agent4 != -1:
                    update_q_table(Q_table=Q_table_agent4, state=current_state_agent4, action=action_agent4,
                                   reward=reward_agent4, new_state=agent4_pos,
                                   learning_rate=ql_params.learning_rate, discount_factor=ql_params.discount_factor)

            if (agent1_reached_goal and agent2_reached_goal and 
                agent3_reached_goal and agent4_reached_goal):
                success_count += 1
            ...
            if (total_reward_agent1 < penalties.min_cumulative or
                total_reward_agent2 < penalties.min_cumulative or
                total_reward_agent3 < penalties.min_cumulative or
                total_reward_agent4 < penalties.min_cumulative):
        ...
        reward_stats["agent4"]["rewards"].append(total_reward_agent4)
        ...
        success_rate_history.append(success_count / episode)
        

        ql_params.exploration_rate = max(ql_params.min_exploration_rate,
                                         ql_params.exploration_rate * ql_params.exploration_decay)
        ql_params.temperature = max(ql_params.min_temperature,
                                    ql_params.temperature * ql_params.temperature_decay)

        if episode % 100 == 0:
            logging.info(f"Episode {episode}/{simulation_params.episodes} completed. "
                         f"Successes: {success_count}, Collisions: {collision_count}, "
                         f"Exploration Rate: {ql_params.exploration_rate:.4f}, Temperature: {ql_params.temperature:.4f}")
        ...
        for (x, y), count in visited_states_agent4.items():
            total_visits_agent4[x, y] += count
...
def animate_simulation(paths, title):
    ...
    agent4_path, agent4_cumulative_rewards = paths.agent4_run

    max_frames = max(len(agent1_path), len(agent2_path), len(agent3_path), len(agent4_path))
    ...
    ax.scatter(grid_settings.agent4_victory_zone[1], grid_settings.agent4_victory_zone[0], color="orange",
               label="Agent 4 Victory Zone", s=200, marker="*", edgecolors='black', linewidths=1.5)

    agent1_line, = ax.plot([], [], 'b-', lw=2, alpha=0.7)
    agent2_line, = ax.plot([], [], 'g-', lw=2, alpha=0.7)
    agent3_line, = ax.plot([], [], 'm-', lw=2, alpha=0.7)
    agent4_line, = ax.plot([], [], 'c-', lw=2, alpha=0.7)
    ...
    agent4_marker, = ax.plot([], [], 'co', markersize=10, label="Agent 4 Position")

    step_text = ax.text(0.5, -0.1, '', transform=ax.transAxes, fontsize=12, ha='center')
    reward_text_agent1 = ax.text(0.02, 0.92, '', transform=ax.transAxes, fontsize=10, ha='left', color='green')
    reward_text_agent2 = ax.text(0.98, 0.92, '', transform=ax.transAxes, fontsize=10, ha='right', color='blue')
    reward_text_agent3 = ax.text(0.5, 0.92, '', transform=ax.transAxes, fontsize=10, ha='center', color='magenta')
    reward_text_agent4 = ax.text(0.5, 0.82, '', transform=ax.transAxes, fontsize=10, ha='center', color='orange')
    steps_text_agent1 = ax.text(0.02, 0.85, '', transform=ax.transAxes, fontsize=10, ha='left', color='green')
    steps_text_agent2 = ax.text(0.98, 0.85, '', transform=ax.transAxes, fontsize=10, ha='right', color='blue')
    steps_text_agent3 = ax.text(0.5, 0.85, '', transform=ax.transAxes, fontsize=10, ha='center', color='magenta')
    steps_text_agent4 = ax.text(0.5, 0.78, '', transform=ax.transAxes, fontsize=10, ha='center', color='orange')
    ...
    def update(frame):
        ...
        if frame < len(agent4_path):
            agent4_line.set_data([pos[1] for pos in agent4_path[:frame + 1]],
                                 [pos[0] for pos in agent4_path[:frame + 1]])
            agent4_marker.set_data([agent4_path[frame][1]], [agent4_path[frame][0]])
        ...
        if frame < len(agent4_cumulative_rewards):
            reward_text_agent4.set_text(f"Agent 4 Reward: {round(agent4_cumulative_rewards[frame], 2)}")
            steps_text_agent4.set_text(f"Agent 4 Steps: {frame + 1}")

        if (frame < len(agent1_path) and frame < len(agent2_path) and 
            frame < len(agent3_path) and frame < len(agent4_path)):
            d12 = manhattan_distance(agent1_path[frame], agent2_path[frame])
            d13 = manhattan_distance(agent1_path[frame], agent3_path[frame])
            d14 = manhattan_distance(agent1_path[frame], agent4_path[frame])
            d23 = manhattan_distance(agent2_path[frame], agent3_path[frame])
            d24 = manhattan_distance(agent2_path[frame], agent4_path[frame])
            d34 = manhattan_distance(agent3_path[frame], agent4_path[frame])
            collision_occurred = (d12 < 2 or d13 < 2 or d14 < 2 or d23 < 2 or d24 < 2 or d34 < 2)
        else:
        ...
        return (agent1_line, agent2_line, agent3_line, agent4_line,
                agent1_marker, agent2_marker, agent3_marker, agent4_marker,
                step_text, reward_text_agent1, reward_text_agent2, reward_text_agent3, reward_text_agent4,
                steps_text_agent1, steps_text_agent2, steps_text_agent3, steps_text_agent4, collision_text)
    ...
    gif_filename = "Four_Agents_Plots/" + title.replace(" ", "_") + ".gif"
...
def plot_heatmap(
    ...
    fig.tight_layout()
    fn = "Four_Agents_Plots/" + title.replace(" ", "_") + ".pdf"
...
def plot_heatmaps():
    ...
    total_visits_agent4[grid_settings.agent4_victory_zone] = success_count
    ...
    plot_heatmap(total_visits_agent4.T, grid_settings.size, "Agent 4 Visitation Frequencies", cmap='Purples', annotations=True)
    ...
    max_q_agent4 = np.flip(np.max(Q_table_agent4, axis=2))
    ...
    plot_heatmap(max_q_agent4, grid_settings.size, "Agent 4 Maximum Q-values", cmap='Blues', annotations=True)
...
def perform_risk_analysis():
    ...
    agent4_avg_reward = np.mean(reward_stats["agent4"]["rewards"]) if reward_stats["agent4"]["rewards"] else 0
    ...
    if successful_paths:
        ...
        agent4_avg_success_reward = np.mean([path.agent4_run[1][-1] for path in successful_paths])
    else:
        agent1_avg_success_reward = "N/A"
        agent2_avg_success_reward = "N/A"
        agent3_avg_success_reward = "N/A"
        agent4_avg_success_reward = "N/A"
    data = [
        ...
        ["Average Reward (Agent 4)", round(agent4_avg_reward, 2) if reward_stats["agent4"]["rewards"] else "N/A"],
        ...
        ["Average Accumulated Reward (Agent 4) Over Successful Runs", 
         round(agent4_avg_success_reward, 2) if isinstance(agent4_avg_success_reward, float) else agent4_avg_success_reward],
    ...
    if successful_paths:
        ...
        agent4_final_rewards = [path.agent4_run[1][-1] for path in successful_paths]
        ...
        plt.plot(runs, agent4_final_rewards, label='Agent 4', color='orange')
        ...
        plt.savefig("Four_Agents_Plots/Final_Cumulative_Rewards.pdf", format="pdf")
        plt.show()
    else:
...
def enhanced_risk_analysis(burn_in: int = 0):
    ...
    print(tabulate(steps_data, headers=headers_steps, tablefmt="pretty"))
    print("\nAverage Steps to Victory (LaTeX):")
    print(tabulate(steps_data, headers=headers_steps, tablefmt="latex"))
    ...
    print(tabulate(qvalue_data, headers=headers_qvalues, tablefmt="pretty"))
    print("\nAverage Maximum Q-Values per State (LaTeX):")
    print(tabulate(qvalue_data, headers=headers_qvalues, tablefmt="latex"))
    ...
    visitation_data = []
    visit_matrices = [total_visits_agent1, total_visits_agent2, total_visits_agent3, total_visits_agent4]
    for i, agent in enumerate(agents):
    ...
    print(tabulate(visitation_data, headers=headers_visitation, tablefmt="pretty"))
    print("\nVisitation Entropy (LaTeX):")
    print(tabulate(visitation_data, headers=headers_visitation, tablefmt="latex"))
...
def plot_learning_curves():
    ...
    plt.savefig("Four_Agents_Plots/Learning_Curves.pdf", format="pdf")
    plt.show()
...
def main():
    ...
    if successful_paths:
        most_successful = max(successful_paths, key=lambda x: (x.agent1_run[1][-1] + x.agent2_run[1][-1] +
                                                               x.agent3_run[1][-1] + x.agent4_run[1][-1]))
